<?php
/*temp*/shell_exec('echo " [ wp_db.php running ] " >> webhook.log');
function wp_db($branch, $dir_proj){
    ...
    if(file_exists($wp_file) && is_file($wp_file) && is_readable($wp_file)) {
        ...
        @fclose($file);
/*temp*/shell_exec('echo " [ config exists ] " >> webhook.log');
...
/*temp*/shell_exec('echo " [ config info recieved ] " >> webhook.log');
...
/*temp*/shell_exec('echo " [ siteurl added ] " >> webhook.log');
            }
        }
    }
    if($wp_db_creds){
...
/*temp*/shell_exec('echo " [ wp_db.php ran ] " >> webhook.log');